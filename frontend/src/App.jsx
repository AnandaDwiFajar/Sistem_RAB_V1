...
function App() {
  ...
  useEffect(() => {
    ...
    if (!unitsDocRef) return;
    
    const unsubscribe = onSnapshot(unitsDocRef, async (docSnap) => {
        if (docSnap.exists()) {
            setUserUnits(docSnap.data().list || []);
        } else {
            await setDoc(unitsDocRef, { list: DEFAULT_PRIMARY_INPUT_UNITS });
            setUserUnits(DEFAULT_PRIMARY_INPUT_UNITS);
        }
    }, (error) => {
  ...
  useEffect(() => {
    if (!isAuthReady || !userId) { if(currentView === 'materialPrices') setIsLoading(false); return; }
    const pricesCollection = getMaterialPricesCollection();
    if (!pricesCollection) { if(currentView === 'materialPrices') setIsLoading(false); return; }
    if(currentView === 'materialPrices') setIsLoading(true); 
    const unsubscribe = onSnapshot(query(pricesCollection), (snapshot) => {
      setMaterialPrices(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
      if(currentView === 'materialPrices') setIsLoading(false); 
    }, (error) => { console.error("Error fetching material prices:", error); showToast('error', `Error fetching prices: ${error.message}`); if(currentView === 'materialPrices') setIsLoading(false);});
    return () => unsubscribe();
  }, [isAuthReady, userId, getMaterialPricesCollection, currentView]);
 ...
 useEffect(() => {
  ...
  const handleSavePrice = async () => {
    if (!priceFormData.name || !priceFormData.unit || !priceFormData.price) { showToast('error', 'All price fields are required.'); return; }
    const pricesCollection = getMaterialPricesCollection();
    ...
    try {
      if (editingPrice) {
        await setDoc(doc(pricesCollection, editingPrice.id), newPriceData);
  ...
  const handleAddNewUnit = async () => {
    ...
    if (userUnits.map(u => u.toLowerCase()).includes(newUnitName.trim().toLowerCase())) {
    ...
    const updatedUnits = [...userUnits, newUnitName.trim()];
    try {
        await setDoc(unitsDocRef, { list: updatedUnits }, { merge: true });
        setNewUnitName('');
  ...
  const handleDeleteUnit = async (unitToDelete) => {
    const isUsedInPrices = materialPrices.some(price => price.unit === unitToDelete);
    ...
    if (!await window_confirm(`Are you sure you want to delete the unit "${unitToDelete}"? This cannot be undone.`)) return;
    ...
    try {
        await setDoc(unitsDocRef, { list: updatedUnits }, { merge: true });
        showToast('success', `Unit "${unitToDelete}" deleted.`);
        if (priceFormData && priceFormData.unit === unitToDelete) {
            setPriceFormData(prev => ({...prev, unit: userUnits[0] || ''}));
            setUnitSelectionMode('select');
        }
